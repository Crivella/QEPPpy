...
def sum_pdos(atoms="", path=".", plot=False, pfile=True):
	...
	res_orb = None
	for j,atom in enumerate(atoms_l):
		for f in os.listdir(path):
			if "pdos_atm#" in f and "({})".format(atom) in f:
				...
				if res_orb is None:
					n_pt = data.shape[0]
					res_orb = np.zeros((max_atm, n_pt, max_orb+1))
	...
	if not res_orb is None:
		if pfile :
			for atom, j in zip(atoms_l, range(len(atoms_l))):
				...
				np.savetxt(fname=fname, X=res_orb[j,:,:], fmt="%13.8f"+"".join('%14g'*max_orb), header=head)
