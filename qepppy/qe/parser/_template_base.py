				...
				if isinstance(e, dict):
					...
					if arr:
						...
						if len(val) != arr or any(a==None or a =='' for a in val):
							...
							ret1 = False
				if isinstance(e, list):
					return _validate_syntax_(e, lvl+1, arr)
				if isinstance(e, tuple):
					ext = self._get_arr_ext_(e[1], e[2])
			...
			if isinstance(e, dict):
				...
				for el in s:
					...
					if isinstance(el, list):
						if not app:
							break #No optional arguments present
						...
						for el1 in el:
							...
							if isinstance(el1['v'], list):
								el1['v'].append(val)
							else:
								el1['v'] = val
					elif isinstance(el, dict):
						val = self._check_type_(v, el['t'])
						if isinstance(el['v'], list):
							el['v'].append(val)
						else:
							el['v'] = val
					else:
				...
				return True
			if isinstance(e, list): 
				if self._set_line_(line, s=e, card=card):
					return
			if isinstance(e, tuple): 
				if self._set_line_(line, s=e[0], col=e[3], card=card):
					return
		return
